...
/**
 * Created by alapinsk on 05.09.2015.
 */
/**
 *
 * <p>
 * Leandro L. Minku and Xin Yao: DDD: A New Ensemble Approach for Dealing with
...
public class DiversityForDealingWithDrifts extends AbstractClassifier {
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        boolean prediction = Utils.maxIndex(hNl.getVotesForInstance(inst)) == trueClass;

        this.driftDetectionMethod.input(prediction ? 0.0 : 1.0);
        this.drift = DriftDetectionMethodClassifier.DDM_INCONTROL_LEVEL;
        if (this.driftDetectionMethod.getChange()) {
            this.drift = DriftDetectionMethodClassifier.DDM_OUTCONTROL_LEVEL;
        }
        if (this.driftDetectionMethod.getWarningZone()) {
            this.drift = DriftDetectionMethodClassifier.DDM_WARNING_LEVEL;
        }
        switch (this.drift) {
            case DriftDetectionMethodClassifier.DDM_WARNING_LEVEL:
                ...
                break;
            case DriftDetectionMethodClassifier.DDM_OUTCONTROL_LEVEL:
                ...
                break;
            case DriftDetectionMethodClassifier.DDM_INCONTROL_LEVEL:
